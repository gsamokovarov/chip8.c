...
However, running @cmake .@ at the project root directory will result in a lot of cruft files needed by the CMake build process. A better idea is to run:
...
That will keep the CMake specific files in a convenient location. Once @make@ has been run, use @sudo make install@ to install it.
...
SDL is the default backend, you don't need to explicitly specify it. It uses the "SDL":http://www.libsdl.org library display the CHIP-8 graphics and feed it with input from the keyboard.
...
The terminal backend uses escape symbols to render dynamic content straight to the terminal. It requires a terminal that is at least 65x32 render the content correctly.

This backend does not support input.
